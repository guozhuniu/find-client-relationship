...
try:
    ...
    logger.info("IntimacyAnalyzer imported successfully")
except ImportError as e:
    logger.error(f"Error: Could not import IntimacyAnalyzer: {e}")
...
try:
    ...
    conn.close()
    logger.info(f"Data loaded successfully: {len(companies)} companies, {len(people)} people, {len(relationships)} relationships")
except Exception as e:
...
try:
    ...
    logger.info("Organization data generated successfully")
except Exception as e:
    logger.error(f"Error generating organization data: {e}")

# 初始化数据分析器
analyzer = None
if analyzer_available:
    try:
        logger.info("Initializing IntimacyAnalyzer...")
        analyzer = IntimacyAnalyzer()
        analyzer.build_network(people, relationships)
        logger.info("IntimacyAnalyzer initialized successfully")
    except Exception as e:
        logger.error(f"Error initializing IntimacyAnalyzer: {e}")
        analyzer = None
...
def get_companies():
    logger.info('Received request for /api/companies')
    conn = get_db_connection()
    c = conn.cursor()
    c.execute('SELECT * FROM companies')
    companies = []
...
def create_company():
    logger.info('Received request for POST /api/companies')
...
def update_company(id):
    logger.info(f'Received request for PUT /api/companies/{id}')
    data = request.get_json()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
...
def delete_company(id):
    logger.info(f'Received request for DELETE /api/companies/{id}')
    conn = get_db_connection()
    c = conn.cursor()
...
def get_organization(company_id):
    logger.info(f'Received request for /api/organization/{company_id}')
    conn = get_db_connection()
    c = conn.cursor()
...
def get_relationships(person_id):
    logger.info(f'Received request for /api/relationships/{person_id}')
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
...
def get_network_analysis():
    logger.info('Received request for /api/analysis/network')
    """获取网络分析结果"""
    if not analyzer:
...
def get_top_connections(person_id):
    logger.info(f'Received request for /api/analysis/top-connections/{person_id}')
    """获取某个人的 top N 亲密关系"""
    if not analyzer:
        return jsonify({"error": "Analyzer not available"}), 500
...
def get_predicted_connections(person_id):
    logger.info(f'Received request for /api/analysis/predict-connections/{person_id}')
    """获取预测的人脉关系"""
    if not analyzer:
        return jsonify({"error": "Analyzer not available"}), 500
